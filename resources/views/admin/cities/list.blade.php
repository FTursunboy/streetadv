    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box table-responsive">
                <h4 class="header-title m-t-0 m-b-30">Список городов</h4>
                    ...
                    <tbody>
                        <?php if (count($arrObjects) > 0) : ?>
                            <?php foreach ($arrObjects as $object) : ?>
                                <tr class="js-item-tr-<?= $object->cityID; ?>">
                                    <td style="text-align: center"><?= $object->cityID; ?></td>
                                    <td>
                                        <a href="<?= route('admin_cities_edit', [$object->cityID]); ?>"><?= $object->name; ?></a>
                                    </td>
                                    <td style="text-align: center">
                                        <?= isset($oLanguages[$object->languageID]) ? $oLanguages[$object->languageID]->ru_name : 'не установлен'; ?>
                                    </td>
                                    <td style="text-align: center">
                                        <a href="<?= route('admin_cities_edit', [$object->cityID]); ?>" class="on-default edit-row" title="Редактировать" style="margin-right: 20px"><i class="fa fa-cog fa-lg"></i></a>
                                        <a href="#" class="js-delete-item on-default remove-row" data-id="<?= $object->cityID; ?>" title="Удалить"><i class="fa fa-trash fa-lg"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
...
@section('scripts-bottom')
    ...
    <!-- Custom js files -->
    <script src="<?= asset('assets/admin/js/custom/cities/list.js'); ?>"></script>
@endsection
