...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box">
                <h4 class="header-title m-t-0 m-b-30"><?= $edit == true ? 'Редактирование записи' : 'Новая запись'; ?></h4>
                <div class="row">
                    <div class="col-lg-12">
                        <?php if (Session::has('success')) : ?>
                            <div id="js-alert-block" class="alert alert-success alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                <strong>Отлично!</strong> <?= Session::get('success'); ?> Вы можете вернуться к <a href="<?= route('admin_faqs_list'); ?>" class="alert-link">списку</a> или <a href="#" class="alert-link" data-dismiss="alert" aria-hidden="true">продолжить работать</a>.
                            </div>
                        <?php endif; ?>
                        <form id="js-faqs-edit-form" method="post" class="form-horizontal" role="form" action="<?= route('admin_faqs_edit'); ?>">
                            {{ csrf_field() }}
                            <?php if ($edit == true) : ?>
                                <input id="js-faq-id" data-type="faqs" name="faqID" type="hidden" value="<?= $object->faqID; ?>">
                            <?php endif; ?>
                            <div class="form-group">
                                <label class="col-md-2 control-label">Заголовок <span style="color: red;">*</span></label>
                                <div class="col-md-10">
                                    <input id="title" name="title" type="text" class="form-control" value="<?= isset($object->title) ? $object->title : ''; ?>" required>
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-md-2 control-label">Описание <span style="color: red;">*</span></label>
                                <div class="col-md-10">
                                    <textarea id="description" name="description" class="form-control" style="min-height: 250px" required><?= isset($object->description) ? $object->description : ''; ?></textarea>
                                </div>
                            </div>
                            <div class="form-group">
...
@section('scripts-bottom')
    ...
    <script type="text/javascript" src="<?= asset('assets/admin/js/custom/faqs/edit.js'); ?>"></script>
@endsection
