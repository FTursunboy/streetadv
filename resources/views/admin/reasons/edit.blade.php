...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box">
                <h4 class="header-title m-t-0 m-b-30"><?= $edit == true ? 'Редактирование причины' : 'Новая причина'; ?></h4>
                <div class="row">
                    <div class="col-lg-12">
                        <?php if (Session::has('success')) : ?>
                            <div id="js-alert-block" class="alert alert-success alert-dismissable">
                                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                                <strong>Отлично!</strong> <?= Session::get('success'); ?> Вы можете вернуться к <a href="<?= route('admin_reasons_list'); ?>" class="alert-link">списку</a> или <a href="#" class="alert-link" data-dismiss="alert" aria-hidden="true">продолжить работать</a>.
                            </div>
                        <?php endif; ?>
                        <form id="js-reasons-edit-form" method="post" class="form-horizontal" role="form" action="<?= route('admin_reasons_edit'); ?>">
                            {{ csrf_field() }}
                            <?php if ($edit == true) : ?>
                                <input name="feedbackReasonID" type="hidden" value="<?= $object->feedbackReasonID; ?>">
                            <?php endif; ?>
                            <div class="form-group">
                                <label class="col-md-2 control-label">Название причины <span style="color: red;">*</span></label>
                                <div class="col-md-10">
                                    <input id="reason_type" name="reason_type" type="text" class="form-control" value="<?= isset($object->reason_type) ? $object->reason_type : ''; ?>" required>
                                </div>
                            </div>
                            <div class="form-group">
...
@section('scripts-bottom')
    ...
    <script type="text/javascript" src="<?= asset('assets/admin/js/custom/reasons/edit.js'); ?>"></script>
@endsection
