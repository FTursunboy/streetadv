    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box table-responsive">
                <h4 class="header-title m-t-0 m-b-30">Список причин</h4>
                    ...
                    <tbody>
                        <?php if (count($arrObjects) > 0) : ?>
                            <?php foreach ($arrObjects as $object) : ?>
                                <tr class="js-item-tr-<?= $object->feedbackReasonID; ?>">
                                    <td style="text-align: center"><?= $object->feedbackReasonID; ?></td>
                                    <td>
                                        <a href="<?= route('admin_reasons_edit', [$object->feedbackReasonID]); ?>"><?= $object->reason_type; ?></a>
                                    </td>
                                    <td style="text-align: center">
                                        <?= isset($oLanguages[$object->languageID]) ? $oLanguages[$object->languageID]->ru_name : 'не установлен'; ?>
                                    </td>
                                    <td style="text-align: center">
                                        <a href="<?= route('admin_reasons_edit', [$object->feedbackReasonID]); ?>" class="on-default edit-row" title="Редактировать" style="margin-right: 20px"><i class="fa fa-cog fa-lg"></i></a>
                                        <a href="#" class="js-delete-item on-default remove-row" data-id="<?= $object->feedbackReasonID; ?>" title="Удалить"><i class="fa fa-trash fa-lg"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
...
@section('scripts-bottom')
    ...
    <!-- Custom js files -->
    <script src="<?= asset('assets/admin/js/custom/reasons/list.js'); ?>"></script>
@endsection
